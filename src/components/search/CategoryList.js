...
class CategoryList extends React.Component {
  ...
  renderCategories() {
    const content = this.props.pages.map(page => <PageListItem key={page.rootOfCategory} page={page} />)
    if (this.props.pages.length === 0) {
      content.push(
        <div key="no-data" className="tile is-parent column has-text-grey-light">
          No pages for the selected categories found
        </div>,
      )
    }
